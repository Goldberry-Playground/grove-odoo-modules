...
- Implements a 5-zone × 2-tier (bareroot/potted) shipping rate engine over 22 green states; rates live in `data/shipping_rates.json` (maintained by the daily rate-checker — see `scripts/rate_check/`)
...
│   │   └── zip_usda_zone.csv            # PHZM 2023 ZIP→USDA-zone matrix (~15k rows, 22-state trim)
│   ├── docs/
