...
_MODULE_PATH = os.path.join(
    os.path.dirname(__file__), "..", "models", "shipping_zones.py"
)
...
class TestShippingZoneEngineContract(unittest.TestCase):
    ...
    def test_there_are_exactly_twelve_zones(self):
        self.assertEqual(len(sz.ZONE_IDS), 12)
    ...
    def test_flat_base_rate(self):
        with _temp_table({"WV": "zone_1"}, {"zone_1": {"base": 8.0}}):
            self.assertEqual(sz.compute_shipping_rate("WV"), 8.0)

    def test_per_pound_surcharge(self):
        with _temp_table({"CA": "zone_5"}, {"zone_5": {"base": 10.0, "per_lb": 0.5}}):
            # 10 base + 0.5 * 6 lbs = 13.00
            self.assertEqual(sz.compute_shipping_rate("CA", weight=6.0), 13.0)

    def test_free_over_threshold(self):
        rule = {"zone_1": {"base": 8.0, "free_over": 75.0}}
        with _temp_table({"WV": "zone_1"}, rule):
            self.assertEqual(sz.compute_shipping_rate("WV", subtotal=80.0), 0.0)
            self.assertEqual(sz.compute_shipping_rate("WV", subtotal=20.0), 8.0)
...
class TestShippingZoneTableCoverage(unittest.TestCase):
    ...
    def test_every_mapped_zone_has_a_rate(self):
        for state, zone in sz.ZONE_BY_STATE.items():
            self.assertIn(
                zone, sz.ZONE_RATES, f"state {state} maps to {zone} with no rate rule"
            )
            self.assertIn(zone, sz.ZONE_IDS, f"{zone} is not one of the 12 zone ids")
    ...
    def test_every_rate_rule_targets_a_real_zone(self):
        for zone in sz.ZONE_RATES:
            self.assertIn(zone, sz.ZONE_IDS, f"rate rule for unknown zone {zone}")
...
class _temp_table:
    ...
    def __enter__(self):
        self._saved_state = dict(sz.ZONE_BY_STATE)
        self._saved_rates = dict(sz.ZONE_RATES)
